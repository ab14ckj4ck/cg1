...
namespace
{
	...
	class BVHBuilder
	{
		...
		void unpackTriangles(Triangle* triangles, int* material_ids, TriangleIterator triangles_begin, TriangleSentinel triangles_end)
		{
	...
	public:
		...
		BVHNode* createLeaf(TriangleIterator triangles_begin, TriangleSentinel triangles_end)
		{
			...
			auto triangles = make_unique_default<Triangle[]>(num_triangles);
...
void SceneBuilder::addSphere(const float3 c, float r, const std::string& material)
{
	*extendBuffer(spheres, num_spheres++, 1) = { c, r, lookupMaterialID(material) };
}
...
void SceneBuilder::addModel(const std::filesystem::path& filename, const float4x4& M, const std::string& material)
{
	...
	std::transform(begin(obj_triangles), end(obj_triangles), extendBuffer(triangles, num_triangles, size(obj_triangles)), [offset = static_cast<int>(num_vertices)](const Triangle& t) -> Triangle
	{
...
Scene SceneBuilder::finish()
{
	...
	return { std::move(positions), std::move(normals), std::move(texcoords), num_vertices, /*std::move(triangles), std::move(material_ids), num_triangles,*/ std::move(bvh), std::move(planes), num_planes, std::move(spheres), num_spheres, std::move(cones), num_cones, std::move(materials), num_materials, std::move(textures), num_textures };
}
