...
bool Task::readCachedTask(const std::filesystem::path& scene_cache_file_name)
{
	...
	num_ssaa_samples = read<std::int16_t>(file);
	std_dev = read<std::int16_t>(file);
...
void Task::defineCanvas(int width, int height, const float3& background_color, int ssaa_samples, float std_dev)
{
	this->canvas_width = width;
	this->canvas_height = height;
	this->background_color = background_color;
	this->num_ssaa_samples = ssaa_samples;
...
void Task::defineSphere(const float3& c, float r, const std::string& material) {
	std::get<SceneBuilder>(scene).addSphere(c, r, material);
}
...
void Task::render(const std::filesystem::path& output_file, int res_x, int res_y, int max_bounces, int num_threads)
{
	...
	auto framebuffer = raytracer.render(res_x > 0 ? res_x : canvas_width, res_y > 0 ? res_y : canvas_height, std::get<Scene>(scene), camera, data(lights), size(lights), background_color, max_bounces, num_threads, num_ssaa_samples, std_dev);
