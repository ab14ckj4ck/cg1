...
class Scene {
  ...
  //const Plane* findClosestHitPlanes(const float3& p, const float3& d,
                                    //const Plane* planes, std::size_t num_planes, float& t) const;
  ...
  bool intersectsPlane(const float3& p, const float3& d, float t_min,
                       float t_max) const;
  const Sphere* findClosestHitSphere(const float3 &p, const float3 &d,
                                   float& t) const;
  bool intersectsSphere(const float3 &p, const float3 &d, float t_min, float t_max) const;
  ...
  HitPoint hit(const Plane& plane, const float3& p, const float3& d,
               float t) const;  
  HitPoint hit(const Sphere& sphere, const float3& p, const float3& d,
               float t) const;
 ...
 public:
  Scene(std::unique_ptr<float3[]>&& positions,
        ...
        std::unique_ptr<Sphere[]>&& spheres, std::size_t num_spheres,
