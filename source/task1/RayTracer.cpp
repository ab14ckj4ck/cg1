...
image2D<float3> RayTracer::render(int res_x, int res_y, const Scene& scene, const Camera& camera, const Pointlight* lights, std::size_t num_lights, const float3& background_color, int max_bounces, int num_threads, int ssaa_samples_per_pixel, float kernel_standard_deviation)
{
	image2D<float3> framebuffer(res_x, res_y);

	std::cout << "rendering..." << std::endl;

	launchTiled([&framebuffer, &scene, &camera, lights, num_lights, &background_color, max_bounces, ssaa_samples_per_pixel, kernel_standard_deviation](int left, int top, int right, int bottom, int n, int N, int thread_id)
	{
		::render(framebuffer, left, top, right, bottom, scene, camera, lights, num_lights, background_color, max_bounces, ssaa_samples_per_pixel, kernel_standard_deviation);
