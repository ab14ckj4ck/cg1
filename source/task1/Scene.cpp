...
Scene::Scene(std::unique_ptr<float3[]> &&positions, std::unique_ptr<float3[]> &&normals, std::unique_ptr<float2[]> &&texcoords, std::size_t num_vertices, /*std::unique_ptr<triangle_t[]>&& triangles, std::unique_ptr<int[]>&& material_ids, std::size_t num_triangles,*/ BVHRoot &&bvh, std::unique_ptr<Plane[]> planes, std::size_t num_planes,
						 std::unique_ptr<Sphere[]> &&spheres, size_t num_spheres, std::unique_ptr<Cone[]> &&cones, std::size_t num_cones, std::unique_ptr<Material[]> &&materials, std::size_t num_materials, std::unique_ptr<image2D<std::uint32_t>[]> &&textures, std::size_t num_textures)
...
HitPoint Scene::hit(const Sphere &sphere, const float3 &p, const float3 &d, float t) const
{
	float3 position = p + t * d;

	auto &mat = materials[sphere.material];

	float3 normal = normalize(position - sphere.c);
...
HitPoint Scene::hit(const Triangle &tri, int material_id, float lambda_1, float lambda_2) const
{
...
{
	return findClosestHitPlanes(p, d, planes.get(), num_planes, t);
}

bool Scene::intersectsPlane(const float3 &p, const float3 &d, float t_min, float t_max) const
{
	return intersectsRayPlane(p, d, planes.get(), num_planes, t_min, t_max);
}


const Sphere* Scene::findClosestHitSphere(const float3 &p, const float3 &d, float &t) const
{
	Sphere *hit = nullptr;

	for (auto sphere = &spheres[0]; sphere != &spheres[0] + num_spheres; ++sphere)
	{
		if (float t_hit; intersectRaySphere(p, d, sphere->c, sphere->r, t_hit) && t_hit < t)
		{
			t = t_hit;
			hit = sphere;
		}
...
bool Scene::intersectsSphere(const float3 &p, const float3 &d, float t_min, float t_max) const
{
	for (auto sphere = &spheres[0]; sphere != &spheres[0] + num_spheres; ++sphere)
		if (float t; intersectRaySphere(p, d, sphere->c, sphere->r, t) && t >= t_min && t <= t_max)
			return true;
	return false;
}

std::tuple<const Triangle *, int> Scene::findClosestHitTriangle(const float3 &p, const float3 &d, float &t, float &lambda_1, float &lambda_2) const
{
...
std::optional<HitPoint> Scene::findClosestHit(const float3 &p, const float3 &d) const
{
	...
	auto sphere = findClosestHitSphere(p, d, t_min);
...
bool Scene::intersectsRay(const float3 &p, const float3 &d, float t_min, float t_max) const
{
	...
	return bvh->intersectsRay(p, d, positions.get(), t_min, t_max) || intersectsPlane(p, d, t_min, t_max) || intersectsCone(p, d, t_min, t_max) || intersectsSphere(p, d, t_min, t_max);
}
