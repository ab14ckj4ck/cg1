...
namespace
{
	...
	public:
		...
		Parser* endObject(std::size_t num_members) override
		{
			...
			builder.defineSphere(c, r, material);
			return ret;
		}
	};
	...
	public:
		...
		Parser* parseKey(std::string&& str) override
		{
			...
			else if (str == "ssaa_samples")
				return &consumers.emplace<config::NumberConsumer<int>>(this, ssaa_samples);
			else if (str == "std_dev")
				return &consumers.emplace<config::NumberConsumer<float>>(this, std_dev);
		...
		Parser* endObject(std::size_t num_members) override
		{
			builder.defineCanvas(width, height, background_color, ssaa_samples, std_dev);
			return ret;
		}
	};
