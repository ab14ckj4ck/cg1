...
struct TaskBuilder
{
	virtual void defineCanvas(int width, int height, const float3& background_color, int ssaa_samples, float std_dev) = 0;
	virtual void definePlane(const float4& p, const std::string& material) = 0;
	virtual void defineSphere(const float3& c, float r, const std::string& material) = 0;
