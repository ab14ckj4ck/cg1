...
namespace
{
	...
	public:
		...
		void visitLeaf(const Triangle* triangles, std::size_t num_triangles, const int* material_ids) override
		{
...
BVHRoot deserializeBVH(std::istream& file)
{
	...
	while (true)
	{
		...
		{
			auto triangles = num_triangles ? make_unique_default<Triangle[]>(num_triangles) : nullptr;
...
namespace
{
																...
																float &lambda_1, float &lambda_2)
	{
		...
		for (auto tri = triangles; tri < triangles + num_triangles; ++tri)
		{
			if (float t_found, u_found, v_found;
					intersectRayTriangle(p, d, vertices[(*tri)[0]], vertices[(*tri)[1]],
															vertices[(*tri)[2]], t_found, u_found, v_found))
			{
				if (t_found < t)
				{
					t = t_found;
	...
	{
		for (auto tri = triangles; tri < triangles + num_triangles; ++tri)
			if (float t, lambda_1, lambda_2;
					intersectRayTriangle(p, d, vertices[(*tri)[0]], vertices[(*tri)[1]],
															vertices[(*tri)[2]], t, lambda_1, lambda_2) &&
					t >= t_min && t <= t_max)
				return true;
		return false;
	}

	void lalala(const Triangle*& tri, float& t, float& lambda_1, float& lambda_2, int& mat, const float3& p, const float3& d, const float3* vertices, const BoundingBox& bb, const BVHNode& node)
	{
...
std::tuple<const Triangle*, int> BVHInnerNode::findClosestHit(const float3& p, const float3& d, const float3* vertices, float& t, float& lambda_1, float& lambda_2) const
{
	const Triangle* tri_l = nullptr;
...
BVHLeafNode::BVHLeafNode(std::unique_ptr<Triangle[]>&& triangles, std::size_t num_triangles, std::unique_ptr<int[]>&& material_ids)
...
std::tuple<const Triangle*, int> BVHLeafNode::findClosestHit(const float3& p, const float3& d, const float3* vertices, float& t, float& lambda_1, float& lambda_2) const
{
	if (auto tri = ::findClosestHitTriangles(p, d, &triangles[0], num_triangles, vertices, t, lambda_1, lambda_2))
...
{
	return ::intersectsRayTriangle(p, d, &triangles[0], num_triangles, vertices, t_min, t_max);
}
...
std::tuple<const Triangle*, int> BVHRoot::findClosestHit(const float3& p, const float3& d, const float3* vertices, float& t, float& lambda_1, float& lambda_2) const
{
	if (intersect(bb, p, d, 0.0f))
